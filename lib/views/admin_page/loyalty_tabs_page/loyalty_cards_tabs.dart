...
class LoyaltyCardsTabsState extends State<LoyaltyCardsTabs> {
                    ...
                    child: Container(
                      child: Wrap(
                        ...
                        children: [
                          ...
                          Visibility(
                            visible: loyaltyCardFromDB.type != 0 ? true : false,
                            child: LoyaltyEditButton(
                              iconData: Feather.percent,
                              text: "Hediye Yüzdesi Ayarı",
                              onClick: () => changePercentage(context),
                            ),
                          ),
                          LoyaltyEditButton(
  ...
  changePercentage(BuildContext context) {
    showDialog(
        context: context,
        builder: (_) => PercentageSettingDialog(
              initialPercentage: loyaltyCardFromDB.percentage.toString(),
            )).then((value) {
              if(value!=null){
                loyaltyCardFromDB.percentage = double.parse(value);
              }
    });
  }
